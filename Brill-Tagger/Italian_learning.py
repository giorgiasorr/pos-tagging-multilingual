...
def create_corpus_C(training_file):
    ...
    with open(training_file, "r") as file:
        with open(os.path.join(sys.path[0], "corpus_without_tags_italian.txt"), "w") as fp:
            for line in file:
                if line.strip():
                    word, tag = line.split()
...
def calculate_possible_tags(word_to_tag):
    ...
    new_dict = {}
    with open(os.path.join(sys.path[0], "possible_tags_italian.txt"), "w") as fp:
...
All_possible_tags = os.path.join(sys.path[0], "possible_tags_italian.txt")
Corpus_without_tags = os.path.join(sys.path[0], "corpus_without_tags_italian.txt")
...
Annotated_corpus_majority_tags = os.path.join(sys.path[0], "annotated_corpus_italian.txt")
...
output_file_path = os.path.join(sys.path[0], "final_data_italian.txt")
