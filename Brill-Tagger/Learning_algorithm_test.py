...
# context=[Prev_word, current_rord, next_word]
tagB_list = []
for i in confusion_table1:
    tagB_list.append(i[1])
    # print(i[1])
...
# def find_tags_context(list1, list2):
#     tags_set = set(list1)  # Convert list1 to a set for faster lookup
#     contexts = []  # List to store the contexts

#     for sublist in list2:
#         for i, item in enumerate(sublist):
#             if item[1] in tags_set:  # Check if the tag is in tags_set
#                 previous_word = sublist[i-1][1] if i > 0 else None
#                 next_word = sublist[i+1][1] if i < len(sublist) - 1 else None
#                 contexts.append((previous_word, item[1], next_word))

#     return contexts

# # Example usage
# list1 = ["NNP", "word1", "word2", "word3"]
# list2 = [[('in', 'IN'), ('Europe', 'NNP')], [('Friday', 'NNP'), ('.', '.')]]

# result = find_tags_context(list1, list2)
# print(result)
...
# # def organize_tags_context(list1, list2):
# #     tags_dict = {tag: [] for tag in list1}  # Initialize a dictionary with empty lists for each tag

# #     for sublist_index, sublist in enumerate(list2):
# #         for i, item in enumerate(sublist):
# #             if item[1] in tags_dict:  # Check if the tag is in tags_dict
# #                 previous_word = sublist[i-1][1] if i > 0 else None
# #                 next_word = sublist[i+1][1] if i < len(sublist) - 1 else None
# #                 tags_dict[item[1]].append((previous_word, item[1], next_word))

# #     # Construct organized_dict, skipping empty lists
# #     organized_dict = {}
# #     for tag, contexts in tags_dict.items():
# #         nested_lists = []
# #         for sublist_index, sublist in enumerate(list2):
# #             nested_sublist = []
...
# #                 if item[1] == tag:
# #                     if contexts:
# #                         nested_sublist.append(contexts[sublist_index])
# #                     else:
# #                         nested_sublist.append([])
# #                 else:
# #                     nested_sublist.append([])
# #             if any(nested_sublist):
# #                 nested_lists.append(nested_sublist)
# #         if nested_lists:
# #             organized_dict[tag] = nested_lists

# #     return organized_dict

# # # Example usage
# # list1 = ["IN", "NNP", "word1", "word2", "word3"]
# # list2 = [[('in', 'IN'), ('Europe', 'NP')], [('in', 'IN'), ('Europe', 'NNP')] ,[('in', 'IN'), ('Europe', 'NNP')], [('Friday', 'NNP'), ('.', '.')]]

# # result = organize_tags_context(list1, list2)
# # print(result)
...
def organize_tags_context(list1, list2):
    ...
    for tag, contexts in tags_dict.items():
        nested_lists = []
        context_index = 0  # Add an index to keep track of contexts in the flat list
        for sublist in list2:
            nested_sublist = []
            for item in sublist:
                if item[1] == tag:
                    if context_index < len(contexts):
                        nested_sublist.append(contexts[context_index])
                        context_index += 1
                    else:
                        nested_sublist.append([])
                else:
...
result = organize_tags_context(tagB_list, final_parsed)
# print(result)
...
def remove_empty_lists(d):
    if isinstance(d, dict):
        ...
        return [remove_empty_lists(x) for x in d if x != []]
    else:
        return d
...
def analyze_patterns(data):
    ...
    for centered_word, context_list in data.items():
        patterns = [(prev, centered_word, next_) for pattern in context_list for prev, _, next_ in pattern]
        ...
        # Remove duplicates, keeping only one of each pattern
        unique_sorted_patterns = [pattern for pattern, count in sorted_patterns]
        
        result[centered_word] = unique_sorted_patterns
    
    return result

data = {
    'IN': [[(None, 'IN', 'NNP')]],
    'NNP': [[('IN', 'NNP', None)], [('IN', 'NNP', None)], [(None, 'NNP', '.')]]
}
